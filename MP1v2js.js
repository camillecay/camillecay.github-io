...
function getOldData(text)
{ 

    const editEducForm = document.querySelector('#editEducForm');
        db.collection('educations').doc(text).update({

        school: document.getElementsByClassName('Eschool').textContent = editEducForm.educEditschool.value
...
const editButton = document.querySelector('#educData');
            ...
            db.collection('educations').get().then(snapshot => {
                ...
                educContent.appendChild(educ_div);
                    ...
                    editButton.addEventListener('click', (event) => {
                        ...
                        //get the document id, pass it sa function, burahin lahat ng laman, then ilagay lahat ng sinulat sa form :D
                    })
                })
...
function hobbycloseNav() {
  ...
  function educopenEdit() {
    document.getElementById("educeditBar").style.width = "450px";
  

}

function educcloseEdit() {
    document.getElementById("educeditBar").style.width = "0";
  }